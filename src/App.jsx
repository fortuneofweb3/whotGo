...
import { calculateLevel, getGameXPReward } from './utils/achievementService';
...
const App = () => {
    ...
    try {
      ...
      if (loginResult.exists && loginResult.profile) {
        ...
        return {
          ...
          achievementsUnlocked: loginResult.profile.achievements || [],
          claimedAchievements: loginResult.profile.achievements || [],
  ...
  const initializeAchievements = async (userData) => {
    ...
    try {
      // Get achievement data directly from Honeycomb
      ...
      // Get achievements directly from Honeycomb profile
      const honeycombAchievements = honeycombProfile.achievements || [];
      
      // Import achievement definitions from the service
      const { ACHIEVEMENT_DEFINITIONS, getAchievementProgress } = await import('./utils/achievementService');
      
      // Create achievements list with progress from Honeycomb
      const achievementsList = ACHIEVEMENT_DEFINITIONS.map(achievement => {
        const progress = getAchievementProgress(achievement.id, userData);
        const unlocked = honeycombAchievements.includes(achievement.id);
        ...
        return {
          ...
          unlocked: unlocked,
          claimed: unlocked, // Achievements are automatically claimed when unlocked
          icon: '🏆', // Default icon
    ...
    if (currentRoom) {
      ...
      setGameData(prevData => {
        ...
        if (remainingPlayers.length <= 1) {
          ...
          // Handle achievement system for multiplayer game end (Firebase-first approach)
          if (winner && currentUser && publicKey) {
            setTimeout(async () => {
              try {
                const isWinner = winner.id === currentUser.id;
                
                // Update Firebase first with game statistics
                const userRef = ref(db, `users/${currentUser.id}`);
                const currentUserData = currentUser;
                
                // Calculate XP and achievements locally (Firebase-first)
                const baseXP = isWinner ? 50 : 10;
                const roundBonus = (nextRoundGameData.roundNumber || 1) * 5;
                const totalXP = baseXP + roundBonus;
                
                const newXP = (currentUserData.xp || 0) + totalXP;
                ...
                const gamesPlayed = (currentUserData.gamesPlayed || 0) + 1;
                const gamesWon = (currentUserData.gamesWon || 0) + (isWinner ? 1 : 0);
                
                // Simple achievement checks
                if (gamesWon === 1 && !currentAchievements.includes(0)) newAchievements.push(0); // First Victory
                if (gamesPlayed >= 100 && !currentAchievements.includes(4)) newAchievements.push(4); // Century Club
                if (gamesWon >= 50 && !currentAchievements.includes(5)) newAchievements.push(5); // Ultimate Champion
                ...
                // Then sync to Honeycomb
                if (publicKey && wallet && signMessage) {
                  try {
                    await syncFirebaseToHoneycomb(publicKey, updatedUserData, wallet, signMessage);
                  } catch (error) {
  ...
  const handleRoundEnd = async gameData => {
    ...
    // Update game statistics for this round (count as a game) - Firebase-first approach
    if (currentUser) {
